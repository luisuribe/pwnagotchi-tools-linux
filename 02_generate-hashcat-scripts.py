...
fullHashcatPath = "/opt/hashcat/hashcat-6.2.5/"

# the FULL path to where your wordlists are saved
fullWordListPath = "/handshakes/wordlists/"
...
def generateHashcatScript(filename):
    ...
    outputPath = '--potfile-path "' + fullProjectPath + 'handshakes/hashcat-potfile.txt" -o "' + fullProjectPath + 'handshakes/hashcat-output.txt"'
    rulePath = fullProjectPath + "handshakes/rules/"
    ...
    f = open("./handshakes/scripts/" + fileId + ".sh", "w")
    ...
    sessionScripts.append(fileId + ".sh")
...
def generateScriptForBatch():
    ...
    if(len(sessionScripts) > 1):
        f = open("./handshakes/scripts/batches/batch-" + str(randint(1000, 9999)) + ".sh", "w")
        batchScript = ""
        for script in sessionScripts:
            batchScript += 'sh "' + fullProjectPath + 'hashcat/scripts/' + script + '"\n'
        f.write(batchScript)
        f.close()
